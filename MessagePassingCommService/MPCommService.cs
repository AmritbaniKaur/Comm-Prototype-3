...
namespace MessagePassingComm
{
    ...
    public class Sender
    {
            ...
            while (true)
            {
                ...
                if (msg.to == lastUrl)
                {
                    ...
                    catch(Exception ex)
                    {
                        ...
                        {
                            Console.WriteLine("Another Exception: ", ex1);
                        }
                    }
                }
                else
        ...
        {
            // Check if Sender sent file to 'toPath' before this, if yes, use the same channel
            // if the toPath is a new one then create a new channel and send

            if (toEndPoint != lastUrl)
            {
                close();
                createSendChannel(toEndPoint);
                lastUrl = toEndPoint;
            }
